    ...
    <div className={styles.goalPanelContainer}>
        ...
        {(type === "drafts" || type === "goals") && selectedGoal && (
          <>
            <div className={styles.goalPanelInfo}>
              ...
              <p className={styles.goalPanelDescrtiption}>
                {selectedGoal.description}
              </p>
            </div>
            <div className={styles.goalPanelStats}>
              <GoalProgressRing
                taskCount={selectedGoal.taskCount}
                in_progress={selectedGoal.in_progress}
                done={selectedGoal.done}
              />
              <span
                onClick={() => setIsOpen(true)}
                className={styles.goalKebabMenu}
              >
                ⋮
              </span>
            </div>
      ...
      {!selectedGoal && type !== "overall" && (
        <div className={styles.noGoalMessage}>
          No goal selected yet — create a new one or pick from the sidebar to
          begin.
        </div>
      )}
      {selectedGoal && (
